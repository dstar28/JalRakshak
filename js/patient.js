...
function renderVillageRisk() {
  ...
  const container =
    document.getElementById('villageRiskList');
...
async function evaluateSymptoms() {
  ...
  const villageObj =
    villagesData.find(v => v.village === village);

  const villageRisk =
    villageObj ? villageObj.riskScore : 50;


  const data = {

    vomiting:
      document.getElementById('sym-vomiting').checked ? 1 : 0,

    fever:
      document.getElementById('sym-fever').checked ? 1 : 0,

    diarrhea:
      document.getElementById('sym-diarrhea').checked ? 1 : 0,

    dehydration:
      document.getElementById('sym-dehydration').checked ? 1 : 0,

    stomachPain:
      document.getElementById('sym-abdominal').checked ? 1 : 0,
  ...
  try {
    ...
    if (score >= 10) {
      ...
      advice =
        "⚠ Moderate risk. Consult doctor within 24 hours.";
      ...
      advice =
        "✅ Low risk. Rest, hydrate, and monitor symptoms.";
